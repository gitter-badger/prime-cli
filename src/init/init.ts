 ...
 * Strip the standard generator prefixes ("generator-" and "prime-init-"),
 * and extract the remainder of the name (the first part of the string before
 * any colons).
 *
 * Examples:
 *
 *   'generator-prime-init-foo'         === 'foo'
 *   'prime-init-foo'                   === 'foo'
 *   'foo-bar'                            === 'foo-bar'
 *   'generator-prime-init-foo:aaa'     === 'foo'
 *   'prime-init-foo:bbb'               === 'foo'
 *   'foo-bar:ccc'                        === 'foo-bar'
 */
...
function createSelectPrompt(env: YeomanEnvironment): InquirerQuestion {
  ...
  const allGeneratorNames = Object.keys(generators).filter((k) => {
    return k.startsWith('prime-init') && k !== 'prime-init:app';
  });
