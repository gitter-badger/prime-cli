 ...
 * This code may only be used under the BSD style license found at http://prime.github.io/LICENSE.txt
 * The complete set of authors may be found at http://prime.github.io/AUTHORS.txt
 * The complete set of contributors may be found at http://prime.github.io/CONTRIBUTORS.txt
 * Code distributed by Google as part of the prime project is also
 * subject to an additional IP rights grant found at http://prime.github.io/PATENTS.txt
 */
'use strict';
...
suite('init', () => {
  ...
  suite('promptGeneratorSelection', () => {
    ...
    const GENERATORS = [
      {
        generatorName: 'prime-init-plugin:plugin',
        metaName: 'prime-init-plugin',
        shortName: 'plugin',
        description: 'A blank plugin template',
        resolved: 'unknown',
      },
      {
        generatorName: 'prime-init-my-test-app:app',
        metaName: 'prime-init-my-test-app',
        shortName: 'my-test-app',
        description: 'my test app',
      },
      {
        generatorName: 'prime-init-prime-starter-kit-custom-1:app',
        metaName: 'prime-init-prime-starter-kit-1',
        shortName: 'prime-starter-kit-1',
        description: 'PSK 1',
      },
      {
        generatorName: 'prime-init-prime-starter-kit-custom-2:app',
        metaName: 'generator-prime-init-prime-starter-kit-2',
        shortName: 'prime-starter-kit-2',
        description: 'PSK 2',
      },
      {
        generatorName: 'prime-init-custom-build-1:app',
        metaName: 'generator-prime-init-custom-build-1',
        shortName: 'custom-build-1',
        description: 'custom build 1',
      },
      {
        generatorName: 'prime-init-custom-build-2:app',
        metaName: 'prime-init-custom-build-2',
        shortName: 'custom-build-2',
        description: 'custom build 2',
      },
      {
        generatorName: 'prime-init-custom-build-3:app',
    ...
    test('includes user-provided generators in the list when properly installed/registered', () => {
      ...
      let promptStub = sandbox.stub(inquirer, 'prompt').returns(Promise.resolve({generatorName: 'TEST'}));
      helpers.registerDependencies(yeomanEnv, [[helpers.createDummyGenerator(), 'prime-init-custom-template:app']]);
      return polymerInit.promptGeneratorSelection({env: yeomanEnv}).catch((error) => {
        ...
        assert.equal(customGeneratorChoice.value, 'prime-init-custom-template:app');
